    ...
    def __init__(
        ...
        *arg
        ...
        self._combobox = Combobox(
            master=self, width=input_width, values=values, textvariable=textvariable
        )
...
class EntryForm(Frame):
    ...
    def __init__(
        self, label_name: str, input_width: int, master: Misc, value: Variable, *arg
    ):
        super().__init__(master, *arg)
...
class RadioButtonForm(Frame):
    ...
    def __init__(
        self, master: Misc, form_name: str, values: list[tuple[str, str]], init: str
    ):
        ...
        for text, value in values:
            radio_button = Radiobutton(
                master=self, text=text, variable=self.__select_item, value=value
            )
...
def make_check_buttons(master: Misc):
    ...
    for key, var in checkbutton_config.items():
        ...
        chk = tk.Checkbutton(master=master, variable=checkbutton[key], text=key)
        chk.pack(anchor=tk.W, side="top", fill="x")


class CommonInputForm(Frame):
    """
    共通入力項目
    """
    ...
    def __init__(self, master: Misc):
        ...
        self._measure_person = ComboboxForm(
            label_name="測定者",
            input_width=10,
            values=refer_users_table(),
            master=self.form_top,
            textvariable=self._input_measure_person,
        )
...
class ParameterInputsForm(Frame, metaclass=ABCMeta):
    ...
    def __init__(self, master, param_names: list[str], variables: list[Variable]):
        ...
        self.__select_from_template_button = Button(
            ...
            text="テンプレートから選択",
            cursor="hand1",
            command=self.open_select_templete_window,
        )
        ...
        self.__register_templete_button = Button(
            ...
            text="テンプレートに登録",
            cursor="hand1",
            command=self.register_templete,
        )
...
class TempletesWindow(Toplevel, metaclass=ABCMeta):
    def __init__(
        self, master: Misc, main_window: ParameterInputsForm, columns: list[str]
    ):
        ...
        self.select_button = Button(
            self, text="テンプレートを反映", command=self.set_values
        )
    ...
    def set_values(self):
        ...
        if selected_item:
            ...
            self.main_window.recall_templete(
                values
