...
from ..core.measurement import set_folder_func, exc_run_func, stop_func
from ..core.algorithm import MeasureBlock, MeasureList, Cycle
...
class CustomSpinbox(tk.Spinbox):
    ...
    @classmethod
    def get_all(cls) -> List['CustomSpinbox']:
        """全てのインスタンスを取得します。"""
        return cls.instances

    def ref_instances(self) -> List['CustomSpinbox']:
        """インスタンスの参照を取得します。"""
        return CustomSpinbox.instances
    ...
    def del_block(self) -> None:
        ...
        for instance in BlockLabelMain.instances:
            if instance.block.selected:
                MeasureBlock.instances.remove(instance.block)
                del instance.block
                BlockLabelMain.instances.remove(instance)
                instance.destroy()
        if len(MeasureBlock.instances) == 1:
            self.del_button["state"] = tk.DISABLED
...
class WindowSub(tk.Toplevel):
    def __init__(self, measure_list: MeasureList, master: tk.Tk = None):
        """サイクルウィンドウを初期化します。"""
        super().__init__(master)
        self.grab_set()
    ...
    def on_close(self) -> None:
        ...
        for ins in Cycle.instances:
            index_list = [MeasureBlock.instances.index(block) for block in ins.cycle_contents]
...
from algorithm import MeasureBlock, MeasureList, Cycle
...
class BlockLabelMain(tk.Label):
    ...
    def __init__(self, master: tk.Tk, **kwargs):
        ...
        self.block = self.master.measure_list.make_block(SpinboxMain.instances)
    ...
    def open_setting(self, event: tk.Event = None) -> None:
        """設定を開きます。"""
        self.block.select(SpinboxMain.instances)
